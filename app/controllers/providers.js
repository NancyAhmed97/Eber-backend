...
exports.provider_heat_map = async function (req, res) {
    try {
        utils.check_request_params(req.body, [{ name: 'provider_id', type: 'string' }], function (response) {
            if (!response.success) {
                ...
                Country.findOne({countryname: provider_detail.country}).then((country) => {
                    ...
                    if(provider_detail.is_trip.length>0){
                        Trip.findOne({_id: provider_detail.is_trip[0]}).then((trip_detail)=>{
                            Trip_history.findOne({_id: provider_detail.is_trip[0]}).then((trip_history_detail)=>{
                                if(!trip_detail){
                                    trip_detail = trip_history_detail;
                                }
                                if(trip_detail){
                                    ...
                                    User.findOne({_id: trip_detail.user_id}, function(error, user_detail){
                                        ...
                                        res.json({success: true, setting_detail: setting_response, phone_number_min_length: setting_detail.minimum_phone_number_length,
                                                            phone_number_length: setting_detail.maximum_phone_number_length,
                                                            provider_detail: response, trip_detail: trip_details});   
                                    });
                                } else {
                                    res.json({success: true, setting_detail: setting_response,phone_number_min_length: setting_detail.minimum_phone_number_length,
                                                            phone_number_length: setting_detail.maximum_phone_number_length, provider_detail: response});   
                                }
                            });
                        });
                    } else {
