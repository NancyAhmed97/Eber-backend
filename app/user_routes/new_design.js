...
module.exports = function (app) {
    ...
    app.route('/generate_token').post(users.generate_token);
    app.route('/forgot_password_:type').get(users.forgot_password);

    app.route('/guest-user').get(users.guest_user);
    app.route('/track-trip').get(users.track_trip);
    app.route('/get-track-trip-data').post(users.get_track_trip_data);
};