...
exports.login = function (req, res, next) {
    ...
    if (req.body.type == 'user') {
        res.render('user-login-form', { setting_data: setting_detail });
    } else {
        res.render('driver-login-form', { setting_data: setting_detail });
    }
    delete message;
}

exports.generate_token = function(req,res){
    var tokenData  = utils.generate_token();
    res.json(tokenData);
}
...
exports.forgot_password = function (req, res, next) {
    ...
    res.render('forgot_password',{type:type,token:token.data});
}
...
exports.guest_user = async function (req, res) {
    ...
    if (!guest_token) {
        return res.render('guest-user-error');
    }
    return res.render('guest-user', { data: req.body, map_key: setting.web_app_google_key, country, guest_token });
}

exports.track_trip = async function (req, res) {
    let guest_user_token = req.query.token;
    ...
    return res.render('track-trip', { data: req.body, map_key: setting.web_app_google_key, country, guest_token, trip_data, trip_path_data });
}

exports.get_track_trip_data = async function (req, res) {
    let guest_user_token = req.body.guest_user_token;
    let trip_id = req.body.trip_id;
    if (!guest_user_token || !trip_id) {
        return res.json({ success: false });
    }
    let trip_data = await Trip.findOne({ _id: trip_id, user_type_id: guest_user_token });
    let trip_path_data = null;
    if (!trip_data) {
        trip_data = await Trip_history.findOne({ _id: trip_id, user_type_id: guest_user_token });
    }
    if (!trip_data) {
        return res.json({ success: false });
    }
    ...
    return res.json({ success: true, trip_data, trip_path_data })
}