        ...
        if (response.success) {
            ...
            if (typeof req.session.userid != 'undefined') {
                Country.find({}).then((country_list) => {
                    ...
                    Trip_history.aggregate([trip_condition, provider_type_condition,trip_filter,country_filter, city_filter, trip_group_condition, lookup, search, count]).then((array) => {

                        if (array.length == 0) {
                            array = [];
                            res.render('admin_partner_weekly_earning', { detail: array, 'current_page': 1, provider_weekly_analytic: provider_weekly_analytic_data, country_list: country_list, city_list: city_list, type: req.body.type, 'pages': 0, 'next': 1, 'pre': 0, moment: moment, week_start_date_view: week_start_date_view, week_end_date_view: week_end_date_view, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });

                        } else {
                            var pages = Math.ceil(array[0].total / number_of_rec);

                            Trip_history.aggregate([trip_condition, provider_type_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, sort, skip, limit]).then((array) => {
                                ...
                                if (array.length == 0) {
                                    ...
                                    Trip_history.aggregate([trip_condition, provider_type_condition, trip_filter, country_filter, city_filter, trip_group_condition_total]).then((trip_total) => {
                ...
                if (typeof req.session.userid != 'undefined') {
                    ...
                    Country.findOne({_id: mongoose.Types.ObjectId(selected_country)}).then((country) => {
                        ...
                        City.findOne({_id: selected_city}).then((city) => {
                            ...
                            Trip_history.aggregate([trip_condition, provider_type_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, sort]).then((array) => {
                                ...
                                if (array.length == 0) {
                                    ...
                                    Trip_history.aggregate([trip_condition, provider_type_condition, trip_filter, country_filter, city_filter, trip_group_condition, lookup, search, sort]).then((array) => {
                                        ...
                                        if (array.length == 0) {
                                            ...
                                            Trip_history.aggregate([trip_condition, provider_type_condition, trip_filter, country_filter, city_filter, trip_group_condition_total]).then(() => {
                                                ...
                                                ws.cell(1, col++).string(req.__('title_partner_id'));
                                                ...
                                                ws.cell(1, col++).string(req.__('title_partner_profit'));
                                                ws.cell(1, col++).string(req.__('title_pay_to_partner'));
