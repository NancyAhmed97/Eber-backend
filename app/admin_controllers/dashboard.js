        ...
        Trip.aggregate([
            {
                $group: {
                    ...
                    canclled_by_user: {
                        $sum: {
                            $cond: [{
                                $and: [{ $eq: ["$is_trip_cancelled_by_user", 1] },
                    ...
                    cancelled_by_provider: {
                        $sum: {
                            $cond: [{
                                $and: [{ $eq: ["$is_trip_cancelled_by_provider", 1] },
                                { $eq: ["$is_schedule_trip", false] }]
                            }, 1, 0]
                        }
                    },
            ...
            Trip_history.aggregate([
                ...
                Trip.count({}, function (err, total_trips) {
                    Trip_history.count({}, function (err, total_trips_history) {
                        if (err) {
                            console.log(err);
                        } else {
                            ...
                            if (array['total_trips'] != constant_json.ZERO) {
                                ...
                                Trip.aggregate([query], function (err, result) {
                                    if (err) {
                                        ...
                                        Trip_history.aggregate([query], function (err, result) {
                                            if (err) {
                                                console.log(err);
                                            } else {
                                                if (result.length !== 0) {
