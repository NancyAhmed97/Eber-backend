    ...
    if (typeof req.session.partner != "undefined") {
        Partner.findById(req.session.partner._id).then((response) => { 
            ...
            Country.findOne({countryname: response.country}).then((countrydata) => { 
                    ...
                    res.render("partner_profile", {phone_number_min_length: setting_detail.minimum_phone_number_length, phone_number_length: setting_detail.maximum_phone_number_length, is_public_demo: is_public_demo, login1: response});
                    delete message;
            });
        });
    } else {
...
exports.partner_trip_map = function (req, res) {
    if (typeof req.session.partner == 'undefined') {
        ...
        Provider.count({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}).then((provider_count) => { 
            ...
            if (provider_count != 0) {
                ...
                Provider.paginate({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}, options).then((providers) => { 
                    if (providers.docs.length <= 0) {
                        res.render('partner_providers_list', {
                            ...
                            next: next, pre: pre, id: req.session.partner._id, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date
                        });
                        delete message;
                    } else {
                        var is_public_demo = setting_detail.is_public_demo;
                ...
                res.render('partner_providers_list', {
                    ...
                    next: '', pre: '', id: req.session.partner._id, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date
                });
                delete message;
            }
        });
            ...
            if (partnercount != 0) {
                ...
                Partner.paginate({$and: [{$or: [query1, query2, query3, query4, query5, query6]}, query]}, options).then((partnerlist) => { 
                    ...
                    if (partnerlist.docs.length <= 0) {
                        res.render('partner_list', { detail: [], currentpage: partnerlist.page, pages: partnerlist.pages, next: next, pre: pre, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                        delete message;
                    } else {
                            var is_public_demo = setting_detail.is_public_demo;
                            res.render('partner_list', { is_public_demo: is_public_demo, detail: partnerlist.docs, currentpage: partnerlist.page, pages: partnerlist.pages, next: next, pre: pre, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                            delete message;
                    }
...
exports.partner_requests = function (req, res, next) {
    ...
    if (typeof req.session.partner != 'undefined') {
        ...
        Table.aggregate([condition, lookup, unwind, lookup1, search, filter, count]).then((array) => { 
            if (!array || array.length == 0) {
                array = [];
                res.render('partner_request_list', { detail: array, request: request, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
            } else {
                var pages = Math.ceil(array[0].total / number_of_rec);
                Table.aggregate([condition, lookup, unwind, lookup1, search, filter, sort, skip, limit]).then((array) => { 

                    res.render('partner_request_list', { detail: array, request: request, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                });
            }
        }, (err) => {
                                    utils.error_response(err, res)
