    ...
    if (typeof req.session.corporate !== 'undefined') {
        Corporate.findById(req.session.corporate._id).then((response) => { 
            Country.findOne({_id: response.country_id}).then((countrydata) => { 
                ...
                res.render("corporate_profile", {phone_number_min_length: setting_detail.minimum_phone_number_length, phone_number_length: setting_detail.maximum_phone_number_length, login1: response});
                delete message;
            });
        });
    } else {
...
exports.corporate_send_request = function (req, res) {
    if (typeof req.session.corporate != 'undefined') {
...
exports.corporate_remove_user = function (req, res) {
    if (typeof req.session.corporate != 'undefined') {
...
exports.corporate_request = function (req, res) {
    ...
    if (typeof req.session.corporate == 'undefined') {
        ...
        Table.aggregate([filter, condition, lookup, unwind, lookup1, search, count]).then((array) => {
            if (array.length == 0) {
                res.render('corporate_request_list', { detail: array, request: request, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
            } else {
                ...
                Table.aggregate([filter, condition, lookup, unwind, lookup1, search, sort, skip, limit]).then((array) => {
                    res.render('corporate_request_list', { detail: array, request: request, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                });
            }
        });
    }
    ...
    if (typeof req.session.corporate == 'undefined') {
        ...
        Country.findOne({_id: req.session.corporate.country_id}).then((country_data) => { 
            ...
            Trip.aggregate([corporate_type_condition, condition, condition1, condition2, condition3, condition4, lookup, unwind, search, filter, count]).then((array) => { 
                ...
                if (array.length == 0) {
                    res.render('corporate_future_request_list', { detail: array, timezone: country_data.countrytimezone, 'current_page': 1, 'pages': 0, 'next': 1, 'pre': 0, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                } else {
                    var pages = Math.ceil(array[0].total / number_of_rec);
                    Trip.aggregate([corporate_type_condition, condition, condition1, condition2, condition3, condition4, lookup, unwind, search, filter, sort, skip, limit]).then((array) => { 
                        ...
                        res.render('corporate_future_request_list', { detail: array, timezone: country_data.countrytimezone, 'current_page': page, 'pages': pages, 'next': next, 'pre': pre, moment: moment, sort_field, sort_order, search_item, search_value, filter_start_date, filter_end_date });
                    }, (err) => {
                    utils.error_response(err, res)
                    });
                }
...
exports.corporate_trip_map = function (req, res) {
    if (typeof req.session.corporate == 'undefined') {
                ...
                if (index == array.length - 1) {
                    wb.write('data/xlsheet/' + time + '_corporate_request.xlsx', function (err) {
                        if (err) {
                            ...
                            var url = req.protocol + "://" + req.get('host') + "/xlsheet/" + time + "_corporate_request.xlsx";
                            res.json(url);
                            setTimeout(function () {
...
exports.genetare_future_request_excel = function (req, res) {
    ...
    if (typeof req.session.corporate == 'undefined') {
        ...
        Country.findOne({_id: req.session.corporate.country_id}).then(() => { 
            Trip.aggregate([corporate_type_condition, condition, condition1, condition2, condition3, condition4, lookup, unwind, search, filter]).then((array) => { 
                var date = new Date();
                var time = date.getTime();
                var wb = new xl.Workbook();
                var ws = wb.addWorksheet('sheet1');
